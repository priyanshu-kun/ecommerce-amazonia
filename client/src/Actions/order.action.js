...
export const createOrder = (orderItems) => async (dispatch,getState) => {
    ...
    try {
        ...
        const {data} = await Axios.post("https://priyanshu-amazonia.herokuapp.com/api/order",{orderItems},{
            headers: {
...
export const detailsOrder = (id) => async (dispatch,getState) => {
    ...
    try {
        const {signIn: userInfo} = getState();
        const {data} = await Axios.get(`https://priyanshu-amazonia.herokuapp.com/api/order/${id}`,{
            headers: {
                Authorization: `Bearer ${userInfo.userInfo.token}`
            }
        })
...
export const payOrder = (order,paymentResult) => async (dispatch,getState) => {
    try {
        ...
        const {signIn: userInfo} = getState();
        const {data} = await Axios.put(`https://priyanshu-amazonia.herokuapp.com/api/order/${order._id}/pay`,paymentResult,{
            headers: {
                Authorization: `Bearer ${userInfo.userInfo.token}`
            }
        })
...
export const MyOrdersList = () => async (dispatch,getState) => {
    ...
    try {
        const {signIn: userInfo} = getState();
        const {data} = await Axios.get("https://priyanshu-amazonia.herokuapp.com/api/order/exect/my",{
            headers: {
                Authorization: `Bearer ${userInfo.userInfo.token}`
            }
        })
